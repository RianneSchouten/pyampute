...
class mdPatterns():
    ...
    def get_patterns(self, X: Matrix, show_plot: bool = True, show_patterns: bool = True) -> Matrix:
        """Some comments
    ...
    def _calculate_patterns(self, X: pd.DataFrame):
        ...
        group_values.sort_values(by=['zero_count','row_count'],ascending=[True,False],inplace = True)
    ...
    def _make_plot(self):
        ...
        heat_values = group_values.iloc[0:(group_values.shape[0] - 1), 1:group_values.shape[1]-1]
        ...
        ax.set_yticklabels(group_values.iloc[0:(group_values.shape[0]-1), 0]) # first column
        ax.set_yticks(np.arange(-.5, len(heat_values.index), 1), minor=True)

        ax.set_xticks(np.arange(0, len(heat_values.columns), 1))
        ax.set_xticklabels(group_values.iloc[group_values.shape[0]-1, 1:(group_values.shape[1]-1)]) # last row
        ax.set_xticks(np.arange(-.5, len(heat_values.columns), 1), minor=True) 

        by.set_yticks(np.arange(0, (len(heat_values.index) * 2) + 1, 1))
        right_ticklabels = list(group_values.iloc[0:(group_values.shape[0]-1), group_values.shape[1]-1]) # last column
