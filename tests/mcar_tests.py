...
def checks_input_mcar_tests(data):
    ...
    if not any(data.dtypes.values == np.float):
        if not any(data.dtypes.values == np.int):
            print("Error: Dataset can only contain floats and/or ints.")
            return False
...
class McarTests:
    ...
    def mcar_test(self):
        ...
        -------
        p_value: Float
            This value is the outcome of a chi-square statistical test, testing whether
            the null hypothesis ('missingness mech of the incomplete dataset is MCAR')
            can be rejected.
        """
    ...
    def mcar_t_tests(self):
        ...
        for var in vars:
            for tvar in vars:
                ...
                mcar_matrix.loc[var, tvar] = st.ttest_ind(
