...
import pandas as pd
import numpy as np
from mcar_tests import McarTests, checks_input_mcar_tests
...
class TestMcarTests(unittest.TestCase):
    ...
    def test_checks_input_mcar_tests(self):
        ...
        self.assertFalse(
            checks_input_mcar_tests(pd.DataFrame(np.array([[1, np.nan], ["hoi", 4.0]])))
        )
        self.assertFalse(
            checks_input_mcar_tests(pd.DataFrame(np.array([[1, 2], [3.0, 4]])))
        )
        self.assertTrue(
            checks_input_mcar_tests(pd.DataFrame(np.array([[1, np.nan], [3.0, 4.0]])))
        )
